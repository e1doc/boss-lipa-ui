...
const getters = {
  ...
  pisoPayTransactions: (state) => state.pisoPayTransactions,
  currentPisoPayTransaction: (state) => state.currentPisoPayTransaction,
};
...
const mutations = {
  ...
  setPisoPayTransactions: (state, pisoPayTransactions) =>
    (state.pisoPayTransactions = pisoPayTransactions),
  setCurrentPisoPayTransaction: (state, currentPisoPayTransaction) =>
    (state.currentPisoPayTransaction = currentPisoPayTransaction),
};
...
const actions = {
  ...
  async getAllUserPisoPayTransactions({ commit, getters }, { page = 1 }) {
    try {
      const response = await axios.get(
        `${baseUrl}api/pisopay-transaction-list/?page=${page}&filter=${getters.currentType}`,
        {
          headers: { Authorization: `jwt ${getters.authToken}` },
        }
      );
      await commit("setPageCount", response.data.total_pages);
      await commit("setPisoPayTransactions", response.data.results);
    } catch (err) {
      err.response ? console.log(err.response) : console.log(err);
    }
  },
  async getAllPisoPayTransactions({ commit, getters }, { page = 1 }) {
    try {
      const response = await axios.get(
        `${baseUrl}/staff/pisopay-transaction-list/?page=${page}&filter=${getters.currentType}&search=${getters.transactionSearch}`,
        {
          headers: { Authorization: `jwt ${getters.authToken}` },
        }
      );
