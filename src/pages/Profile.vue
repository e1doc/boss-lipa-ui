...
export default {
  ...
  async mounted() {
    const userChoice = await this.showRPTConfirmationDialog("Title", "Text");

    if (!userChoice) {
      this.$router.push({ name: "RealPropertyEnrollment" });
    }
  },
  methods: {
    async showRPTConfirmationDialog(title, text) {
      try {
        if (
          ...
          const result = await this.$swal({
            title: "Real Property Enrollment",
            text: "Have RPT already?",
            icon: "question",
            showCancelButton: true,
            confirmButtonText: "Yes",
            cancelButtonText: "No",
          });
          return result.value; // Resolve with the user's choice (true for "Yes", false for "No" or closing the dialog)
        } else {
          return true; // Resolve as true if the conditions are not met (no dialog shown)
        }
      } catch (error) {
        console.error("Error displaying confirmation dialog:", error);
        return false; // Resolve as false if there is an error
      }
    },
  },
};
